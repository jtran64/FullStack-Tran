...
abstract class API
{
    ...
    public function __construct() {
		...
		$this->logger->do_log($this->request_uri, 'uri');
...
class MyAPI extends API
{
    ...
    {
		//$this->logger->do_log($this->request,"request");
		$max_id = $this->mh->get_max_id($this->mdb, $this->mh->collection, '_id');
		$user = $this->request;
		$user[_id] = $max_id + 1;
		$this->logger->do_log($user,"user to be inserted");
		$result = $this->mh->insert([$user]);
    ...
    {
		$this->logger->do_log($this->request[data],"data passed to update");
		foreach ($this->request[data] as $key => $value) {
			$this->logger->do_log([$key=>$value]);
			if($value != '')
				$result = $this->mh->update($this->request[_id], [$key=>$value]);
        }
		//$result = $this->mh->update($this->request[_id], [first=>"john"]);
		return $result;
    }
    ...
    {
		$this->logger->do_log($this->request[_id],"data");
		if ($this->method == "DELETE"){
			$users = array();
            if(count($this->request) > 0){               
				if(count($this->request) == 1){
					$users = $this->mh->query(["_id"=>(int)$this->request[_id]]);
